...
class Client(threading.Thread):
    ...
    def make_query(self):
        ...
        if self.args.with_predicate:
            ...
            predicate = ['{}.id <= {}'.format(i, predicate_value) for i in from_stmt]
...
def run_exp(args, mode):
    ...
    if args.install:
        if args.run_mode == AIDE:
            #args.compile_option += "-DSMARTSSD -DSMARTSSD_DBUF -DVERSION_INDEX -DNUM_VI_PAGE=25600 -DSMARTSSD_TIME_PROFILE" # for no ndp
            ...
            if args.gdb:
                ...
                args.compile_option += " -DSMARTSSD_DEBUG -DSLEEP_ON_ASSERT"
