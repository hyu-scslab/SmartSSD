...
if [ $is_ch = true ] ; then
    ./ch_benchmark_ndp.py ${CH_THREAD_COUNT} ${PGHOST} ${RAMPUP_TIME} ${file_name} >> results/ch_benchmarks_${file_name}.log &
    ch_pid=$!
    echo ${ch_pid}
fi
...
if [ $is_tpcc = true ] ; then
    ...
    (cd HammerDB-$version && time ./hammerdbcli auto ../run_ch.tcl | tee "../results/hammerdb_run_${file_name}.log")
    ...
    sleep ${DEFAULT_CH_RUNTIME_IN_SECS:-3600}
fi
