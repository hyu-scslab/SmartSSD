...
class Client(threading.Thread):
    def __init__(self, client_id, autocommit, result_file, aide, group):
...
if __name__ == "__main__":
    ...
    set_group=int(sys.argv[6])

    if ISAIDE == "true":
        aide_thread_count = 1

    file_suffix=sys.argv[7] + "_"
    ...
    aide_file_path = "results/aide_ch_queries_{}_aide_worker#{}.txt"
    ...
    short_olap = [Client(client_id = i, autocommit=True, result_file=os.path.join(short_file_path.format(file_suffix, i)), aide=False, group=-1) \
            for i in range(1, short_thread_count + 1)]

    long_olap = [Client(client_id = i, autocommit=False, result_file=os.path.join(long_file_path.format(file_suffix, i)), aide=False, group=-1) \
            for i in range(1, long_thread_count + 1)]

    aide_olap = [Client(client_id = set_group, autocommit=False, result_file=os.path.join(aide_file_path.format(file_suffix, i)), aide=True, group = set_group) \
    aide_olap = [Client(client_id = i, autocommit=False, result_file=os.path.join(aide_file_path.format(file_suffix)), aide=True, group=set_group) \
