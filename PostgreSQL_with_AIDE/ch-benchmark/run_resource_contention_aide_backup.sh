...
if [ $is_ch = true ] ; then
  python3 ./ch_benchmark_resource_contention_aide_perq.py ${LONG_CH_THREAD_COUNT} ${SHORT_CH_THREAD_COUNT} ${PGHOST} ${OLAP_RAMPUP_TIME} ${AIDE} ${GROUP} ${QUERY_NUM} ${OUTPUT_DIR} ${file_name} >> ${OUTPUT_DIR}/ch_benchmarks_${file_name}.log &
  ch_pid=$!
  echo ${ch_pid}
fi
