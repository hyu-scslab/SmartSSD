...
#pg = ['/opt/p3608-1/jongbin/SOSP21/PostgreSQL/pgsql/bin/psql', '-P', 'pager=off', '-v', 'ON_ERROR_STOP=1', '-h', coord_ip, '-c', query]
    pg = [os.environ['PGHOME'] + '/psql', '-P', 'pager=off', '-v', 'ON_ERROR_STOP=1', '-h', coord_ip, '-c', query]
    #pg = ['/opt/p3608-1/jongbin/j3vm/j3vm/postgres/inst/bin/psql', '-P', 'pager=off', '-v', 'ON_ERROR_STOP=1', '-h', coord_ip, '-c', query]
    #pg = ['/opt/p3608-1/jongbin/j3vm/postgres/inst/bin/psql', '-P', 'pager=off', '-v', 'ON_ERROR_STOP=1', '-h', coord_ip, '-c', query]
    #pg = ['/opt/p3608-1/jongbin/j3vm/vdriver/postgres/inst/bin/psql', '-P', 'pager=off', '-v', 'ON_ERROR_STOP=1', '-h', coord_ip, '-c', query]
...
if __name__ == "__main__":
    ...
    thread_count = 1
    coord_ip = "localhost"
    initial_sleep_in_mins=int(sys.argv[1])
    ...
    print(is_ndp)
    if is_ndp == "YES":
        query_file_path="../ndp_pgsql_script/PostgreSQL/ch-queries/q" + query_number + ".sql"
    elif is_ndp == "NO":
        query_file_path="../ndp_pgsql_script/PostgreSQL/ch-queries-no-hint/q" + query_number + ".sql"
    else:
        assert False
    ...
    for i in range(0,3):
        pg = [os.environ['PGHOME'] + '/psql', '-P', 'pager=off', '-v', 'ON_ERROR_STOP=1', '-h', coord_ip, '-f', query_file_path, '-p', os.environ['PGPORT'], '-d', os.environ['PGDATABASE']]

        start_time = int(round(time.time() * 1000))
        print(pg)
        return_str = subprocess.check_output(pg, universal_newlines=True)
        end_time = int(round(time.time() * 1000))

        with open("results/ch_queries_{}.txt".format(file_suffix), "a") as f:
            f.write("{} finished in {} milliseconds\n".format(int(query_number), end_time - start_time))

        with open("results/ch_queries_results_{}.txt".format(file_suffix), "a") as f:
            f.write("Q{}\n {}\n".format(int(query_number), return_str))
