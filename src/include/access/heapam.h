...
#if defined(SMARTSSD)
	void            *rs_cbuf_dev;   /* buffer from device scan, if any */
	BlockNumber     rs_nitems_dev;
	ItemLength      rs_ilen_dev;
#endif
...
#if defined(SMARTSSD)
	DeviceOffsetNumber rs_vistuples_dev[MaxHeapTuplesPerSegment];   /* their offsets */
#endif
}			HeapScanDescData;
...
#ifdef VERSION_INDEX
extern void HeapTupleSetHintBitsForVi(HeapTupleHeader tuple, Buffer buffer,
									  uint16 infomask, TransactionId xid,
									  bool is_ndp);
#endif /* VERSION_INDEX */
