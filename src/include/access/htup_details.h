...
#ifdef VERSION_INDEX
	uint32 t_page_id;	/* ViPageId */
	uint32 t_offset;	/* ViPageOffset */
#else	/* For testing only, to test the 'real' vanilla, delete this */
	uint32 t_page_id;	/* ViPageId */
	uint32 t_offset;	/* ViPageOffset */
#endif	/* VERSION_INDEX */
...
#ifdef VERSION_INDEX
#define HeapTupleHeaderHasNulls(tup) \
( \
  	((tup)->t_infomask & HEAP_HASNULL) != 0 \
)
#endif	/* VERSION_INDEX */
...
#if defined(SMARTSSD)
#define MaxHeapTuplesPerSegment \
	((int) ((SEGSZ_DEV - HDRSZ_DEV) / \
		(MAXALIGN(SizeofHeapTupleHeader) + sizeof(ItemIdData))))
#endif
