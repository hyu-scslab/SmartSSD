...
#if defined(SMARTSSD) || defined(SMARTSSD_DBUF)
#include "nodes/nodeFuncs.h"
#include "catalog/pg_operator.h" // for Form_pg_operator
#include "utils/syscache.h" // for OPEROID
...
#if defined(SMARTSSD) || defined(SMARTSSD_DBUF)
static void GatherJoinNodes(PlanState *planstate, GatherJoinState *gs);
static void GatherSubjoinNodes(List *plans, GatherJoinState *gs);
static void GatherMemberJoinNodes(PlanState **planstates, int nplans, GatherJoinState *gs);
static bool PrescanNodes(PlanState *planstate, Bitmapset **rels_used);
extern bool planstate_tree_walker(struct PlanState *planstate, bool (*walker)(), void *context);
bool IsSystemQueryPlan(EState *estate);
#endif
#if defined(SMARTSSD_DBUF) && defined(SMARTSSD_DBUF_DEBUG)
int GetUserHeapNum(EState *estate);
#endif
...
#if defined(SMARTSSD) || defined(SMARTSSD_DBUF)
static void 
GatherSubjoinNodes(List *plans, GatherJoinState *gs)
{
	ListCell *lst;
	...
	foreach(lst, plans)
	{
		...
		if (bms_is_member(sp->plan_id, gs->subplans))
			continue;

		gs->subplans = bms_add_member(gs->subplans, sp->plan_id);

		GatherJoinNodes(sps->planstate, gs);
	}
}

/*
 * Gather join nodes to offload from the constituent plans of a ModifyTable, 
 * Append, MergeAppend, BitmapAnd, or BitmapOr node.
 */
static void
GatherMemberJoinNodes(PlanState **planstates, int nplans, GatherJoinState *gs)
{
	int j;

	for (j = 0; j < nplans; j++)
		GatherJoinNodes(planstates[j], gs);
}

static void 
GatherJoinNodes(PlanState *planstate, GatherJoinState *gs)
{
		...
		case T_HashJoin:
			if ((((Join *) plan)->jointype) == JOIN_INNER) { 
				...
				{ 
					...
					if (list_length(planstate->state->es_join_tree)) 
					{ 
						list_free(planstate->state->es_join_tree); 
						planstate->state->es_join_tree = NIL; 
					} 
				} 
				planstate->state->es_join_tree = lcons(planstate, planstate->state->es_join_tree);
			} 
			break;
	...
	if (planstate->initPlan)
		GatherSubjoinNodes(planstate->initPlan, gs);

	/* lefttree */
	if (outerPlanState(planstate))
		GatherJoinNodes(outerPlanState(planstate), gs);

	/* righttree */
	if (innerPlanState(planstate))
		GatherJoinNodes(innerPlanState(planstate), gs);

	/* special child plans */
	switch (nodeTag(plan))
	{
		case T_ModifyTable:
			GatherMemberJoinNodes(((ModifyTableState *) planstate)->mt_plans,
					((ModifyTableState *) planstate)->mt_nplans, gs);
			break;
		case T_Append:
			GatherMemberJoinNodes(((AppendState *) planstate)->appendplans,
					((AppendState *) planstate)->as_nplans, gs);
			break;
		case T_MergeAppend:
			GatherMemberJoinNodes(((MergeAppendState *) planstate)->mergeplans,
					((MergeAppendState *) planstate)->ms_nplans, gs);
			break;
		case T_BitmapAnd:
			GatherMemberJoinNodes(((BitmapAndState *) planstate)->bitmapplans,
					((BitmapAndState *) planstate)->nplans, gs);
			break;
		case T_BitmapOr:
			GatherMemberJoinNodes(((BitmapOrState *) planstate)->bitmapplans,
					((BitmapOrState *) planstate)->nplans, gs);
			break;
		case T_SubqueryScan:
			GatherJoinNodes(((SubqueryScanState *) planstate)->subplan, gs);
			break;
			//case T_CustomScan:
			//	ExplainCustomChildren((CustomScanState *) planstate, gs);
			//	break;
		default:
			break;
	}
	...
	if (planstate->subPlan)
		GatherSubjoinNodes(planstate->subPlan, gs);
}

static bool
PrescanNodes(PlanState *planstate, Bitmapset **rels_used)
{
	Plan *plan = planstate->plan;
	...
	return planstate_tree_walker(planstate, PrescanNodes, rels_used);
}
#endif

#ifdef SMARTSSD
// added by wsul on 220111
struct QProfile profiles[128];
int QProfile_target_pid;
#ifdef WSUL_CTID_TEST
Oid target_version_index = 0;
#endif //WSUL_CTID_TEST
extern int MyProcPid;

bool enable_ndp_offload = false;
int logfile_fd = 0;
...
#if defined(SMARTSSD) || defined(SMARTSSD_DBUF)
// Check if a query plan refers to system catalog.
bool IsSystemQueryPlan(EState *estate)
{
	int rel_idx;

	for (rel_idx = 0 ; rel_idx < estate->es_range_table_size ; rel_idx++)
	{
		if (estate->es_relations[rel_idx] && 
				!IsNDPTargetTable(estate->es_relations[rel_idx]))
		{
			return true;
		}
	}
...
#if defined(SMARTSSD_DBUF) && (SMARTSSD_DBUF_DEBUG)
int GetUserHeapNum(EState *estate)
{
	int rel_idx;
	int num_user_heap = 0;
		...
		bool execute_once)
{
	...
	// added by wsul on 220111
	//int i;
	//struct QProfile profile;
	// added by wsul on 220111

#if defined(SMARTSSD_DBUF_DEBUG) && defined(DBUF_HARD_DEBUG)
	/* 
	 * These are used junt only for dirty buffer id flush
	 */
	Relation test_rel; // relation pointer for test
	BlockNumber nblocks; // total number of blocks of relation
	int rel_idx, num_user_heap;
	uint32 seg_idx; 
...
#endif
	/*
	 * This code segment is inserted just for test.
	 * If query is offloaded, dirty buffer list is flushed within somewhere in
	 * this area. So, dirty buffer flush logic is inserted here.
	 */

	if (operation == CMD_SELECT)
	{
		gs = (GatherJoinState *)palloc0(sizeof(GatherJoinState));
		gs->subplans = NULL;
		gs->need_reset = false;

		// store hash join nodes from leaf to root order
		planstate->state->es_join_tree = NIL;
		planstate->state->es_offload = false;

		if (IsUnderPostmaster && estate && !IsSystemQueryPlan(estate))
		{
			GatherJoinNodes(planstate, gs);
			len = list_length(planstate->state->es_join_tree);
		}
		else
		{
			len = 0;
		...
		num_user_heap = GetUserHeapNum(estate);

		if (len) 
		{
			for (rel_idx = 0 ; rel_idx < num_user_heap ; rel_idx++)
			{
				uint32 max_segno;
				// Get relation
				test_rel = estate->es_relations[rel_idx];

				if (!test_rel)
					continue;

				// Get the total number of blocks in the relation
				nblocks = smgrnblocks(test_rel->rd_smgr, MAIN_FORKNUM);
				max_segno = (nblocks + (RELSEG_SIZE - 1)) / RELSEG_SIZE;

				// get the number of segment in relation from the number of 
				// blocks
				for (seg_idx = 0 ; seg_idx < max_segno ; seg_idx++)
				{
					int flushed_idx;

					/* Entry point of NDP module */
					// get a dirty buffer id's list.	
					if (!IsNDPTargetTable(test_rel))
						continue;
					...
					for (flushed_idx = 0 ; flushed_idx < 32 ; flushed_idx++)
					{
						...
						if (flushed_rel_id_array[flushed_idx] == 
								test_rel->rd_id)
							break;
					}
					...
					flushed_rel_id_array[flushed_idx] = test_rel->rd_id;

					dbuf_list = ndpsmgrdetachdirtybufid(test_rel->rd_id, 
							seg_idx);
					if (dbuf_list)
					{
						fprintf(stderr, "Dirty list flush Rel[%u][%u]\n",  
								test_rel->rd_id, seg_idx);
						FlushSegmentBuffersForNDP(dbuf_list, test_rel->rd_smgr);
					}
				}
			}
		}
	...
	List *tl = NIL; /* List of hj_clause_filter whose filtering and hash join would be offloaded to FPGA */
	...
	/* time profiling code */
	struct timespec start_operation, end_operation; 
	double total_operation_time;
	char log_comment[1024] = {0, };

	clock_gettime(CLOCK_MONOTONIC, &start_operation);
#endif
	if (operation == CMD_SELECT)
	{
		if (!enable_ndp_offload) {
			len = 0;
		} else {
			gs = (GatherJoinState *)palloc0(sizeof(GatherJoinState));
			gs->subplans = NULL;
			...
			{
#ifdef SMARTSSD_DEBUG
				fprintf(stderr, "[SMARTSSD] inappropriate NDP target, %s %d %s\n", 
						__FILE__, __LINE__, __func__);
#endif
				len = 0;
			}
#ifdef SMARTSSD_DEBUG
#ifndef DBUF_HARD_DEBUG
			fprintf(stderr, "[SMARTSSD] join_depth:%d, %s %d %s\n", 
					len, __FILE__, __LINE__, __func__);
#endif
#endif // #ifdef SMARTSSD_DEBUG
			if (len)
			{
				PrescanNodes(planstate, &rels_used);
				rtable_names = select_rtable_names_for_explain(
						planstate->state->es_range_table, rels_used);
				planstate->state->es_deparse_cxt = deparse_context_for_plan_rtable(
						planstate->state->es_range_table, rtable_names); 
				tl = GetOffloadTargets(planstate);
#ifdef SMARTSSD_DEBUG    
				fprintf(stderr, "[SMARTSSD] len:%d, %s %d %s\n", 
						list_length(tl), __FILE__, __LINE__, __func__);
#endif				
				if (list_length(tl))
				{
					planstate->state->es_offload = true;
#ifdef SMARTSSD_DEBUG    
					foreach (lc, tl)
					{
						cf = (s3d_hj_clause_filter *) lfirst(lc);
						fprintf(stderr, "[SMARTSSD] outer:%d, inner:%d\n",
								cf->outer_relid, cf->inner_relid);

						foreach (lc2, cf->filter_list)
						{	
							filter = (s3d_filter *) lfirst(lc2);
							fprintf(stderr, "filter:%d.%d %d, type:%d, len:%d, value:%s, %s %d %s\n",
									filter->relid, filter->attnum, filter->opno, 
									filter->constval->consttype, 
									filter->constval->constlen,
									filter->constval->constvalue ? "not null" : "null", 
									__FILE__, __LINE__, __func__);
						}						
					}  
#endif
					nrel = TransferJoinInfoForNDPwithVersionIndex(planstate, tl, &files);
					list_free(tl);

					// flush data pages and version files by segments for each relation
					if (files) {
						FlushVersionIndexBySegment(planstate, files, nrel);
						pfree(files);
					}
				}				
			}	
...
#ifdef SMARTSSD
	if (planstate->state->es_offload)
	{
		pthread_join(planstate->state->es_s3d_thread, NULL);
		free(planstate->state->es_thread_param);
	}

	list_free(rtable_names);
	list_free(planstate->state->es_join_tree);
	list_free(planstate->state->es_deparse_cxt);
#endif
	// added by wsul on 220111
	/*	if (QProfile_target_pid == MyProcPid) {
			...
			profile = profiles[i];
			if (profile.count) {
			fprintf(stderr, "type: %d %s %lu times %lu us (avg: %lu us)\n",
			i, profile.name, profile.count, profile.time / 1000,
			profile.time / profile.count / 1000);
			}
			}

			memset(&profiles, 0, sizeof(struct QProfile) * 128);
			}*/
	// added by wsul on 220111
#ifdef SMARTSSD_TIME_PROFILE
	if (len && enable_ndp_offload)
	{
		clock_gettime(CLOCK_MONOTONIC, &end_operation);
		total_operation_time = end_operation.tv_sec - start_operation.tv_sec +
			(end_operation.tv_nsec - start_operation.tv_nsec) / 1000000000.0;

		sprintf(log_comment, "%.3lf\n", (double)total_operation_time * 1000);
 ...
 */
void EvalPlanQualInit(EPQState *epqstate, EState *parentestate,
