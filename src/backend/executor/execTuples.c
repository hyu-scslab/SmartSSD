...
#if defined(SMARTSSD)
/* store tuple read from SmartSSD */
static inline void
tts_device_heap_store_tuple(TupleTableSlot *slot, HeapTuple tuple,
	Buffer buffer, bool transfer_pin)
{
	...
	bslot->base.off = 0;
	slot->tts_tid = tuple->t_self;
	/* allocate memory for slot's tupdata so as not to hold a pointer into disk page buffer */
	tts_buffer_heap_materialize(slot);
...
/* store tuple read from SmartSSD */
TupleTableSlot *
ExecStoreDeviceHeapTuple(HeapTuple tuple, TupleTableSlot *slot, Buffer buffer, 
	Block buffer_dev)
{
	/*
	 * sanity checks
	...
	Assert(!BufferIsValid(buffer) && buffer_dev != NULL);

	if (unlikely(!TTS_IS_BUFFERTUPLE(slot)))
		elog(ERROR, "trying to store an on-disk heap tuple into wrong type of slot");
	tts_device_heap_store_tuple(slot, tuple, buffer, false);
