...
#ifdef SMARTSSD
	/* There's a probability wal writer couldn't initialize 
		 NDPmodules. Then, initialize them */
	// NDPSupportModuleInit();
	// NDPSupportModuleDsaInit();
#endif
...
#ifdef SMARTSSD
      // Currently, we don't have dedicate background version index flusher yet.
      // So, we owe flushing version index to walwriter.
			// CheckPointVersionIndex();
#endif
