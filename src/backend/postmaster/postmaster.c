...
#ifdef VERSION_INDEX
#define StartViManager()		StartChildProcess(ViManagerProcess)
#endif	/* VERSION_INDEX */
...
#ifdef VERSION_INDEX
		/* If we have lost the log collector, try to start a new one */
		if (ViManagerPID == 0 && pmState == PM_RUN)
			ViManagerPID = StartViManager();
#endif	/* VERSION_INDEX */
...
#ifdef VERSION_INDEX
		if (ViManagerPID != 0)
			signal_child(ViManagerPID, SIGHUP);
#endif	/* VERSION_INDEX */
	...
	switch (postgres_signal_arg)
	{
		case SIGTERM:
				...
				pmState == PM_HOT_STANDBY || pmState == PM_STARTUP)
			{
				...
				/* and the walwriter too */
				if (ViManagerPID != 0)
					signal_child(ViManagerPID, SIGTERM);
#endif	/* VERSION_INDEX */
...
reaper(SIGNAL_ARGS)
{
	...
	while ((pid = waitpid(-1, &exitstatus, WNOHANG)) > 0)
	{
...
#ifdef VERSION_INDEX
		/*
		 * Was it the version index manager launcher?	Normal exit can be
		...
		if (pid == ViManagerPID)
		{
			...
			if (!EXIT_STATUS_0(exitstatus))
				HandleChildCrash(pid, exitstatus,
								 _("version index manager process"));
			continue;
		}
#endif	/* VERSION_INDEX */
...
#ifdef VERSION_INDEX
	/* Take care of the vimanager too */
	if (pid == ViManagerPID)
		ViManagerPID = 0;
	else if (ViManagerPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) ViManagerPID)));
		signal_child(ViManagerPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
#endif	/* VERSION_INDEX */
...
#ifdef VERSION_INDEX
	if (ViManagerPID != 0)
		signal_child(ViManagerPID, signal);
#endif	/* VERSION_INDEX */
}
	...
	if (pid < 0)
	{
		...
		switch (type)
		{
			...
			case WalReceiverProcess:
				...
				break;
#ifdef VERSION_INDEX
			case ViManagerProcess:
				ereport(LOG,
						(errmsg("could not fork version index manager process: %m")));
				break;
#endif	/* VERSION_INDEX */
			default:
				ereport(LOG,
