...
RelationBuildDesc(Oid targetRelId, bool insertIt)
{
	...
	if (IsNDPTargetTable(relation))
	{
		...
		uint32 hashcode;
		bool tt_found;
		Oid target_rel_id = relation->rd_id;


		hashcode = NDPDBufHashCode(&target_rel_id, TargetTableIDHash);
		alloc_lock = DBufHeapIDLock();
		LWLockAcquire(alloc_lock, LW_EXCLUSIVE);	
		HashInsert(&target_rel_id, hashcode, TargetTableIDHash, &tt_found);

#ifdef SMARTSSD_DBUF_DEBUG	
		if (!tt_found)
			fprintf(stderr, "Rel[%u] is allocated\n", target_rel_id);
#endif
		LWLockRelease(alloc_lock);
	}
#endif
#ifdef SMARTSSD_KT
	/* Setting default version index type. 
		 Currently, version index type is set to type1 
	*/
	relation->rd_vi_type = VI_TYPE1;
	relation->dbuf_list_id = -1;
	//RELKIND_TOASTVALUE,

	/*
	if (IsNDPTargetTable(relation))
		relation->dbuf_list_id = NDPFindDBufIDList(relid, false);
	*/
...
#ifdef SMARTSSD_DBUF
	if (IsNDPTargetTable(rd))
	{
		// Need to optimize!
	 	LWLock * alloc_lock;
		uint32 hashcode;
		bool tt_found;
		Oid target_rel_id = rd->rd_id;

		hashcode = NDPDBufHashCode(&target_rel_id, TargetTableIDHash);
		alloc_lock = DBufHeapIDLock();
		LWLockAcquire(alloc_lock, LW_SHARED);	
		HashLookup(&target_rel_id, hashcode, TargetTableIDHash, &tt_found);
		...
		if (!tt_found)
		{
			...
			LWLockRelease(alloc_lock);
			LWLockAcquire(alloc_lock, LW_EXCLUSIVE);
			HashInsert(&target_rel_id, hashcode, TargetTableIDHash, &re_found);
#ifdef SMARTSSD_DBUF_DEBUG
			if (!re_found)
				fprintf(stderr, "Rel[%u] is allocated\n", target_rel_id);
#endif
...
bool
IsNormalObject(Oid relid)
{
	return relid >= (Oid) FirstNormalObjectId;
}

/*
 * IsNDPTargetTable
 *  Check if relation is NDP target table.
 */
bool
IsNDPTargetTable(Relation rel)
{
	// Non-system heap table is target for NDP
	return (!IsSystemRelation(rel) &&
					rel->rd_node.relNode >= (Oid) FirstNormalObjectId &&
					rel->rd_rel->relkind == RELKIND_RELATION);
}

/*
 * IsNormalHeapObject
 * 06/11 Check if this relation is normal heap relation
 * This function is called in dirty buffer id list module
 * so, it assumed that the funcation is called after 
 * checking if the relation is normal or not.
 * so, first it checks relation is heap
 */
bool
IsNormalHeapObject(Relation rel)
{
	// Non-system heap table is target for NDP
	return rel->rd_rel->relkind == RELKIND_RELATION &&
				 !IsToastClass(rel->rd_rel) &&	
				 rel->rd_node.relNode >= (Oid) FirstNormalObjectId;
}
#endif	/* SMARTSSD || VERSION_INDEX */
...
/*
 * RelationCacheGetVIType
 * Get a version index type of relation
 * Param[in]
 * Relation relation - Relation meta data cache address
 * Return
 * vi_type(int) - version index type of relation 
 */
vi_type
RelationCacheGetVIType(Relation relation)
{
	// return version index type of relation
	return relation->rd_vi_type;
}

Relation 
NDPRelationIdCacheLookup(Oid relid) 
{
	RelIdCacheEnt *hentry; 

	hentry = (RelIdCacheEnt *) hash_search(RelationIdCache, (void *) &(relid),
																				 HASH_FIND, NULL);
	if (hentry)
