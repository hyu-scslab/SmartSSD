...
extern bool enable_ndp_offload;
...
Datum 
pg_enable_ndp(PG_FUNCTION_ARGS) {
	enable_ndp_offload = true;	

	return enable_ndp_offload;
}

Datum
pg_disable_ndp(PG_FUNCTION_ARGS) {
	enable_ndp_offload = false;

	return enable_ndp_offload;
}

Datum
pg_open_ctids(PG_FUNCTION_ARGS)
{
	Oid reloid;
	Relation rel;
...
#ifdef WSUL_CTID_TEST
	target_version_index = reloid;
#endif //WSUL_CTID_TEST

	rel = relation_open(reloid, AccessShareLock);
	RelationOpenSmgr(rel);
#ifdef WSUL_CTID_TEST
	smgropenCTIDs(rel->rd_smgr);
#endif //WSUL_CTID_TEST

	relation_close(rel, AccessShareLock);

	PG_RETURN_INT32(reloid);
}

Datum
pg_close_ctids(PG_FUNCTION_ARGS)
{
	Oid reloid;
	Relation rel;
...
// added by wsul on 220111
#else
Datum pg_enable_ndp(PG_FUNCTION_ARGS) {

	return 0;
}

Datum pg_disable_ndp(PG_FUNCTION_ARGS) {

	return 0;
}

Datum pg_open_ctids(PG_FUNCTION_ARGS) {
	
	return 0;
}
Datum pg_close_ctids(PG_FUNCTION_ARGS) {
	return 0;
}
#endif //SMARTSSD
