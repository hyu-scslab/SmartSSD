...
#if defined(SMARTSSD)
/* Check that the result block header from FPGA appears valid */
bool
BlockIsVerified(char *buf, size_t sz, BlockNumber *pnum, ItemLength *plen)
{
	/*header of n_tuples (4 bytes) | sz_tuple (2 bytes) */
	if ((*pnum = *(BlockNumber *)buf) > 0)
		*plen = *(ItemLength *)(buf + sizeof(BlockNumber));

#ifdef SMARTSSD_DEBUG
	fprintf(stderr, "[SMARTSSD] BlockNum:%u, ItemLen:%hu, sz:%zu, %s %d %s\n",
		*pnum, *plen, sz, __FILE__, __LINE__, __func__);
#endif
	if (*pnum < 0 || (*pnum > 0 && *plen <= 0) ||
		((HDRSZ_DEV + *pnum * *plen) != sz))
		return false;
	return true;
}
#endif
...
#ifdef VERSION_INDEX
void
PageRepairFragmentation(Page page, bool is_ndp)
#else	/* VANILLA */
void
...
#ifdef VERSION_INDEX
		/*
		 * NDP tables should not compactify a fragmented page since version
		 * indexes are pointing them. The .i file will be pointing to wrong
		 * tuple slots.
		 */
		if (!is_ndp)
		{
