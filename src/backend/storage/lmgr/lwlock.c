...
InitializeLWLocks(void)
{
...
#if defined(SMARTSSD_DBUF) && defined(VERSION_INDEX)
	/* Initialize heap id hash's LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS;
	for (id = 0; id < NUM_DBUF_HEAPID_HASH ; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_DBUF_HEAPID);

	/* Initialize buf id chain alloc's LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS 
				+ NUM_DBUF_HEAPID_HASH;
	for (id = 0; id < NUM_DBUF_CHAIN_ALLOC; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_DBUF_CHAIN_ALLOC);

	/* Initialize bufid free stack's LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS 
			 	+ NUM_DBUF_HEAPID_HASH + NUM_DBUF_CHAIN_ALLOC;
	for (id = 0; id < NUM_DBUF_FSTACK; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_DBUF_FSTACK);

	/* Initialize num dbuf list's LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS 
			 	+ NUM_DBUF_HEAPID_HASH + NUM_DBUF_CHAIN_ALLOC + NUM_DBUF_FSTACK;
	for (id = 0; id < NUM_DBUF_LIST ; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_DBUF_CHAIN);

	/* Initialize version index buffer LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS 
				+ NUM_DBUF_HEAPID_HASH + NUM_DBUF_CHAIN_ALLOC + NUM_DBUF_FSTACK 
				+ NUM_DBUF_LIST;
	for (id = 0; id < NUM_VI_BUFFER_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_VI_MAPPING);

	/* Initialize lmgrs' LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS 
				+ NUM_DBUF_HEAPID_HASH + NUM_DBUF_CHAIN_ALLOC + NUM_DBUF_FSTACK 
				+ NUM_DBUF_LIST + NUM_VI_BUFFER_PARTITIONS;
	...
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS 
				 + NUM_DBUF_HEAPID_HASH + NUM_DBUF_CHAIN_ALLOC + NUM_DBUF_FSTACK 
				 + NUM_DBUF_LIST + NUM_LOCK_PARTITIONS;
	for (id = 0; id < NUM_PREDICATELOCK_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_PREDICATE_LOCK_MANAGER);
#elif VERSION_INDEX
	/* Initialize version index buffer LWLocks in main array */
	...
	/* Initialize lmgrs' LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS
		+ NUM_VI_BUFFER_PARTITIONS;
	for (id = 0; id < NUM_LOCK_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_LOCK_MANAGER);

	/* Initialize predicate lmgrs' LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS + NUM_BUFFER_PARTITIONS
		+ NUM_VI_BUFFER_PARTITIONS + NUM_LOCK_PARTITIONS;
	for (id = 0; id < NUM_PREDICATELOCK_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_PREDICATE_LOCK_MANAGER);
#else
	/* Initialize lmgrs' LWLocks in main array */
...
#ifdef SMARTSSD_DBUF
	LWLockRegisterTranche(LWTRANCHE_DBUF_HEAPID, "dbuf_heapid");
	LWLockRegisterTranche(LWTRANCHE_DBUF_CHAIN_ALLOC, "dbuf_chain_alloc");
	LWLockRegisterTranche(LWTRANCHE_DBUF_FSTACK, "dbuf_freenode_stack");
	LWLockRegisterTranche(LWTRANCHE_DBUF_CHAIN, "dbuf_chain");
#endif // #ifdef SMARTSSD_DBUF
#ifdef VERSION_INDEX
	LWLockRegisterTranche(LWTRANCHE_VI_MAPPING, "version_index_buffer_mapping");
	LWLockRegisterTranche(LWTRANCHE_VI_BUFFER_IO, "version_index_buffer_io");
#endif	/* VERSION_INDEX */
