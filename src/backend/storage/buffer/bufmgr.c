...
#ifdef SMARTSSD_DBUF
void
ReservePrivateRefCountEntry2(void)
{
...
#ifdef SMARTSSD_DBUF
extern BufIDNode ndp_stack_base;

bool BufferIsPinned2(int buf)
{
	return BufferIsPinned(buf);
}
#endif
...
static void TerminateBufferIO(BufferDesc *buf, bool clear_dirty,
								uint32 set_flag_bits, bool seg_flush);
#else
static void TerminateBufferIO(BufferDesc *buf, bool clear_dirty,
...
#endif
#ifdef SMARTSSD_DBUF
static void FlushBuffer(BufferDesc *buf, SMgrRelation reln, bool seg_flush);
#else
...
#ifdef SMARTSSD
void ReadBufferFromDevice(void* ndpo, Relation reln, BlockNumber blockNum, 
	Block buf, BlockNumber *nitems, ItemLength *ilen)
{
    size_t ret, sz;
    char *packet = (char *)malloc(SEGSZ_DEV);

    /* read a particular scan/hash results from FPGA into the supplied buffer */
    while ((ret = (size_t)Get_Packets(ndpo, reln->rd_id, packet, blockNum)) == -2)
        ;

    /* in the case of no more results (-1) or empty results (0) */
    if (ret == -1 || ret == 0)
    {
        *nitems = InvalidBlockNumber;
        return;
    }

#ifdef SMARTSSD_DEBUG
    fprintf(stderr, "[SMARTSSD] %dth Packet of table (Id %d, total_size = %ld) from ndp_object, %s %d %s\n",
            blockNum, reln->rd_id, ret, __FILE__, __LINE__, __func__);
#endif
    sz = (size_t)(*((uint32 *)packet) * (uint32)(*((uint16 *)(packet + 4))) + (uint32)6);
    memcpy(buf, packet, sz);
    free(packet);

	/* check for data */
	if (!BlockIsVerified((char *)buf, sz, nitems, ilen)){
		ereport(ERROR, (errcode(ERRCODE_DEV_DATA_CORRUPTED),
			errmsg("invalid block of %u from FPGA that corresponds to relation %s",
			blockNum, relpath(reln->rd_smgr->smgr_rnode, MAIN_FORKNUM))));
	}
}	
#endif
		...
		if (!isExtend)
		{
			...
			*hit = true;
			...
			TRACE_POSTGRESQL_BUFFER_READ_DONE(forkNum, blockNum,
				smgr->smgr_rnode.node.spcNode, smgr->smgr_rnode.node.dbNode,
...
#ifdef SMARTSSD_DBUF
		TerminateBufferIO(bufHdr, false, BM_VALID, false);
#else
...
retry:
	...
	/* Drop tables */
#ifdef SMARTSSD_DBUF
	if (IsNormalObject(buf->tag.rnode.relNode) && (buf_state & BM_DIRTY))
	{
		/* Entry point of NDP module */
		/* Buffer was flushed, so we need to remove its IDs from the list */
		/* pop flushed buffer's id from dirty buffer id lists */
		ndpsmgrpopdirtybufid(buf->buf_id + 1);
	}
#endif
	...
	for (;;)
	{
...

		{
			...
			if (!(old_buf_state & BM_DIRTY) &&
				IsNormalObject(bufHdr->tag.rnode.relNode))
			{
				/* put a buf id of page marked dirty to dirty buf id list */
				ndpsmgrsetdirtybufid(buffer);
			}
...
#ifdef SMARTSSD_DBUF
static void
FlushBuffer(BufferDesc *buf, SMgrRelation reln, bool seg_flush)
#else
static void
...
#endif
{
	 ...
	 */
#ifdef SMARTSSD_DBUF
	TerminateBufferIO(buf, true, 0, seg_flush);
#else
...
#ifdef SMARTSSD_DBUF
/*
 * FlushSegmentBuffersForNDP - flush dirty buffer of segments
 */
void
FlushSegmentBuffersForNDP(FlushDirtyBufIDList dbuf_list, SMgrRelation reln)
{
	int buf_id, idx; 
	BufferDesc *bufHdr;
	uint32 buf_state;
	...
	/* sequential search routine for the arr of the given segment */
	for (idx = 0 ; idx < dbuf_list->num_elem ; idx++)
	{
		// Get the buffer id in dirty buffer list
		buf_id = dbuf_list->dirty_buf_ids_arr[idx];
		// Get the block number in dirty buffer list mapped with buffer id
		blk_num = dbuf_list->blk_num_arr[idx];

		Assert(BufferIsValid(buf_id));
		// Get the buffer descriptor mapped with buf_id
		bufHdr = GetBufferDescriptor(buf_id - 1);

		ReservePrivateRefCountEntry();

		// Acquire lock of buffer descriptor
		buf_state = LockBufHdr(bufHdr);

		/* The dirty bufer list has buf_id which means an index number of 
			 a buffer array.
			 At this point, the actual page pointed by this buf_id may changes. 
			 Or, other processes may have flushed this page. Therefore, 
			 We only flush the page which remains as dirty. */

			// Cond 0. check if other process flush this buffer first.
		if((buf_state & (BM_VALID | BM_DIRTY)) == (BM_VALID|BM_DIRTY) &&
		   // Cond 1. check if the actual page pointed by this buf_id is changed
		   (bufHdr->tag.blockNum == blk_num) &&
		   (bufHdr->tag.rnode.relNode == dbuf_list->relid)) {

			// pin buffer and release lock of buffer descriptor.
			PinBuffer_Locked(bufHdr);

			// Get shared lock to buffer contents
			LWLockAcquire(BufferDescriptorGetContentLock(bufHdr), LW_SHARED);
			// Now we can flush buffer. 
			FlushBuffer(bufHdr, reln, false);
#ifdef SMARTSSD_DBUF_DEBUG
			synced_bufs++;
#endif
			// After flusing buffer, release the buffer content's lock
			LWLockRelease(BufferDescriptorGetContentLock(bufHdr));
			// Reading this buffer is done. Unpin buffer
			UnpinBuffer(bufHdr, true);
		} else {
#ifdef SMARTSSD_DBUF_DEBUG
			dropped_bufs++;
#endif
			// Actual buffer page pointed is changed. 
			// Then, release buffer descriptor lock
			UnlockBufHdr(bufHdr, buf_state);
		}
	}

#ifdef SMARTSSD_DBUF_DEBUG
	Assert(dbuf_list->num_elem == synced_bufs + dropped_bufs);
	fprintf(stderr, "%u[%u] %d : %d + %d\n", dbuf_list->relid, dbuf_list->segno,
			dbuf_list->num_elem, synced_bufs, dropped_bufs);
#endif
	
	// There's probabilities that SMgrRelation data structure 
	// mapped with relation is not opened yet. 
	// We need to allocate and open it.
	if (reln == NULL)
	{
		rnode.relNode = dbuf_list->relid;
		rnode.dbNode = MyDatabaseId;
		rnode.spcNode = MyDatabaseTableSpace;
		// Open SMgr(storage manager) relation data structure
		reln = smgropen(rnode, InvalidBackendId);
	}

	// deallocate resources of dirty buffer IDs lists
	pfree(dbuf_list->dirty_buf_ids_arr);
	pfree(dbuf_list->blk_num_arr);
	pfree(dbuf_list);
}
#endif
	...
	if (RelationUsesLocalBuffers(rel))
	{
		for (i = 0; i < NLocBuffer; i++)
		{
				 ...
				 (BM_VALID | BM_DIRTY)) == (BM_VALID | BM_DIRTY))
			{
				...
				smgrwrite(rel->rd_smgr, bufHdr->tag.forkNum, bufHdr->tag.blockNum,
									localpage, false);
...
MarkBufferDirtyHint(Buffer buffer, bool buffer_std)
{
		...
		(BM_DIRTY | BM_JUST_DIRTIED))
	{
		...
		if (dirtied)
		{
			...
			if (IsNormalObject(
				GetBufferDescriptor(buffer - 1)->tag.rnode.relNode))
			{
				/* put the buf id of page marked dirty to dirty buf id list */
				ndpsmgrsetdirtybufid(buffer);
			}
			if (out_crit)
			{
...
#ifdef SMARTSSD_DBUF
	{
		if (IsNormalObject(buf->tag.rnode.relNode))
		{
			/* Entry point of NDP module */
			/* Buffer was flushed, so we need to remove its IDs from the list */
			/* pop flushed buffer's id from dirty buffer id lists */
...
#ifdef SMARTSSD_DBUF
		TerminateBufferIO(buf, false, BM_IO_ERROR, false);
#else
...
#ifdef SMARTSSD_DBUF
/*
 * NDPReleaseDirtyBufID
 *
 */
void 
NDPReleaseDirtyBufID(DirtyBufIDList dirty_bufid_list, Oid relid, uint32 segno)
{
	BufIDNode dbuf_id_node;
	BufferDesc *bufHdr;
	int pop_node_idx = dirty_bufid_list->head_node_idx;
	int buf;

	dirty_bufid_list->num_elem = 0;
	dirty_bufid_list->head_node_idx = dirty_bufid_list->tail_node_idx = 
	INVALIDIDX;

	/* Make sure we can handle the pin inside the loop */
	ResourceOwnerEnlargeBuffers(CurrentResourceOwner);

	while (pop_node_idx != INVALIDIDX)
	{
		// get address of buf id node
		dbuf_id_node = &ndp_stack_base[pop_node_idx];
		buf = dbuf_id_node->buf_id;
		...
		if (bufHdr->tag.rnode.relNode == relid && 
				bufHdr->tag.blockNum == dbuf_id_node->blk_num &&
				bufHdr->tag.blockNum / RELSEG_SIZE == segno)
		{
			bufHdr->dirty_id_node_idx = INVALIDIDX;
		}
		...
		pop_node_idx = dbuf_id_node->next_node_idx;
	}
}
