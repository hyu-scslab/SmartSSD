...
#ifdef SMARTSSD
/*
 * NDPFileSyncWIthRawFD - synchronize written dirty buffer of segment 
 * with a raw file descriptor of segment file
 */
int
NDPFileSyncWithRawFD(int fd, uint32 wait_event_info)
{
	int returnCode;

	// logging sycn operation will be performed
	DO_DB(elog(LOG, "file sync with raw fd : %d", fd));

	pgstat_report_wait_start(wait_event_info); /* Get stat */
	returnCode = fsync(fd); /*** sync file with raw file descriptor ***/
	pgstat_report_wait_end(); /* Get stat */
