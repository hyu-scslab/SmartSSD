			   ...
			   Relation *Irel, int nindexes, bool aggressive)
{
	...
	fprintf(stderr, "[NDP] vacuum init : %s\n", relname);
	fprintf(stderr, "[NDP] vacuum init : %d\n", nindexes);
#endif
...
#ifdef VERSION_INDEX
		if (IsNDPTargetTable(onerel) && ItemIdHasStorage(itemid))
		{
			HeapTupleHeader tup = (HeapTupleHeader) PageGetItem(page, itemid);
			ViFreeOffset(RelationGetRelid(onerel), blkno / RELSEG_SIZE,
						 tup->t_page_id, tup->t_offset);
		}
#endif	/* VERSION_INDEX */
...
#ifdef VERSION_INDEX
	PageRepairFragmentation(page, IsNDPTargetTable(onerel));
#else
