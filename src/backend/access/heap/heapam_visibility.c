...
#ifdef VERSION_INDEX
static inline void
SetHintBitsForVi(HeapTupleHeader tuple, Buffer buffer,
				 uint16 infomask, TransactionId xid, bool is_ndp)
{
	if (TransactionIdIsValid(xid))
	{
		/* NB: xid must be known committed here! */
	...
	if (is_ndp)
	{
		...
		ViSetHintBits(node.relNode, blknum / RELSEG_SIZE, tuple->t_page_id,
					  tuple->t_offset, infomask);
	}
...
#ifdef VERSION_INDEX
void
HeapTupleSetHintBitsForVi(HeapTupleHeader tuple, Buffer buffer,
						  uint16 infomask, TransactionId xid, bool is_ndp)
{
	SetHintBitsForVi(tuple, buffer, infomask, xid, is_ndp);
}
						 ...
						 Buffer buffer)
{
	...
	if (!HeapTupleHeaderXminCommitted(tuple))
	{
		...
		else if (TransactionIdIsCurrentTransactionId(HeapTupleHeaderGetRawXmin(tuple)))
		{
			...
			{
#ifdef VERSION_INDEX
				SetHintBitsForVi(tuple, buffer, HEAP_XMAX_INVALID,
								 InvalidTransactionId, htup->t_ndp);
#else
				/* deleting subtransaction must have aborted */
				SetHintBits(tuple, buffer, HEAP_XMAX_INVALID,
							InvalidTransactionId);
...
#ifdef VERSION_INDEX
			SetHintBitsForVi(tuple, buffer, HEAP_XMIN_COMMITTED,
							 HeapTupleHeaderGetRawXmin(tuple), htup->t_ndp);
#else
			SetHintBits(tuple, buffer, HEAP_XMIN_COMMITTED,
						HeapTupleHeaderGetRawXmin(tuple));
#endif /* VERSION_INDEX */
		...
		{
#ifdef VERSION_INDEX
			SetHintBitsForVi(tuple, buffer, HEAP_XMIN_INVALID,
							 InvalidTransactionId, htup->t_ndp);
#else
			/* it must have aborted or crashed */
			SetHintBits(tuple, buffer, HEAP_XMIN_INVALID,
...
#ifdef VERSION_INDEX
	SetHintBitsForVi(tuple, buffer, HEAP_XMAX_COMMITTED,
					 HeapTupleHeaderGetRawXmax(tuple), htup->t_ndp);
#else
	SetHintBits(tuple, buffer, HEAP_XMAX_COMMITTED,
				HeapTupleHeaderGetRawXmax(tuple));
#endif /* VERSION_INDEX */
