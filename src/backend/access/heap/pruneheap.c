...
#ifdef VERSION_INDEX
	bool is_ndp = IsNDPTargetTable(relation);
	...
	if (ItemIdIsNormal(rootlp))
	{
		...
		if (HeapTupleHeaderIsHeapOnly(htup))
		{
				...
				== HEAPTUPLE_DEAD && !HeapTupleHeaderIsHotUpdated(htup))
			{
				...
				ndeleted++;
#ifdef VERSION_INDEX
				if (is_ndp && ItemIdHasStorage(rootlp))
				{
					ViFreeOffset(rel_id, blkno / RELSEG_SIZE,
								 htup->t_page_id, htup->t_offset);
				}
#endif	/* VERSION_INDEX */
			}
						...
						OffsetNumber *nowunused, int nunused)
{
	 ...
	 */
#ifdef VERSION_INDEX
	PageRepairFragmentation(page, is_ndp);
#else
