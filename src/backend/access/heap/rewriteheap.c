...
#ifdef VERSION_INDEX
	if (IsNDPTargetTable(state->rs_new_rel))
	{
		/* Build the version index structure and write it to ViPage. */
		ViCtidData vi_ctid;

		ViExtractAndBuildViCtid(state->rs_blockno, page, newoff, &vi_ctid);

		ViPutViCtid(state->rs_new_rel, state->rs_blockno / RELSEG_SIZE,
					page, newoff, &vi_ctid);
	}
#endif	/* VERSION_INDEX */
